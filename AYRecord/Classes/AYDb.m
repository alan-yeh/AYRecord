...
- (BOOL)batch:(NSArray<NSString *> *)sqls{
    NSMutableArray<AYSql *> *sqlArray = [NSMutableArray new];
    for (NSString *sql in sqls) {
        [sqlArray addObject:[AYSql buildSql:sql]];
    }
    return [self batchSqls:sqlArray];
}

- (BOOL)batchSqls:(NSArray<AYSql *> *)sqls{
    if (sqls.count < 1) {
        return YES;
    }
...
- (NSArray<AYDbEntry *> *)findWithSql:(AYSql *)sql{
    AYDbConnection *conn = [_config getOpenedConnection];
    @try {
        AYQueryResultSet *result = [[conn prepareStatement:sql] executeQuery];
...
- (AYDbEntry *)findFirstWithSql:(AYSql *)sql{
